...
(txn_type IN ('FUND TRANSFER','INTEREST CREDIT','ATM WITHDRAWAL','UPI','NEFT','IMPS','CASH'));
ALTER TABLE TXN_LOG RENAME COLUMN txn_type TO txn_channel;
ALTER TABLE TXN_LOG add  txn_type VARCHAR2(50) CHECK (txn_type IN ('DEPOSIT', 'WITHDRAW', 'TRANSFER'));

ALTER TABLE TXN_LOG DROP CONSTRAINT SYS_C008594;--Removed foreign key constraint from from account_id

ALTER TABLE TXN_LOG DROP CONSTRAINT SYS_C008582;--Removed foreign key constraint from To account_id 

DROP INDEX IDX_FROM_ACCT_ID;
DROP INDEX IDX_TO_ACCT_ID;
